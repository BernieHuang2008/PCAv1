...
同一个 Namespace 内的密钥、证书、撤销状态和协议迁移关系构成一个封闭的密码学宇宙。不同 Namespace 之间 **MUST** 被视为完全独立，旧 Namespace 中的信任锚、废止声明或证书链 **MUST NOT** 自动作用于新 Namespace。
...
当 Master Secret 泄露时，合法所有者可以签署废止声明，使当前 Namespace（例如 `PCA-v1/A980...`）进入废止状态。若所有者希望在废止后重建体系，则 **MUST** 生成全新的 Master Secret，并注册或选择一个**全新的 Namespace ID**（例如 `PCA-v2/3F8A...`）。
...
协议迁移采用 `Identity/V1/PCA` 基础设施身份建立信任连续性。
...
Identity/V1/PCA
        │
        ├────────────── Sign ──────────────► PCA-v2 Trust Anchor
        │
        └────────────── Sign ──────────────► PCA-v3 Trust Anchor
```
...
好的。根据你的要求，我对 **第 16 章《紧急废止机制（Emergency Revocation）》** 进行了全面修订。

本次修订的核心逻辑是：
1. **明确废止声明的作用域仅限于特定的 Namespace ID**（而非全局或特定版本号）。
2. **在废止流程中增加对 Namespace 匹配的强制性前置检查**。
3. **补充废止后的合法重建路径**（必须迁移至新 Namespace），并交叉引用新增的 Namespace 章节。

以下是 **修订后的第 16 章完整内容**（你可以直接替换原草案中的对应章节）：

---

# 第 16 章 紧急废止机制（Emergency Revocation）
...
## 16.2 废止声明（Revocation Statement）的格式与作用域
...
```json
{
  ...
  "new_trust_anchor": "Optional: PCA-v2/3F8A..."
}
```

### 16.2.1 作用域强制绑定（Namespace Scope Binding）
...
- 上传至域名根目录下的 `/.well-known/pca/revocation.crl`（具体托管与拉取流程见第 17 章）。
- 在个人网站、社交媒体、邮件列表、PGP 密钥服务器等公开渠道同步发布。
- 必要时，可通过公证机构或区块链时间戳服务固化废止声明的发布时间，以证明废止操作的时效性。

---

## 16.4 验证端检查流程（含 Namespace 前置校验）
...
2. **签名有效性验证（Signature Verification）**：
   - 使用**硬编码的紧急废止公钥**（见 16.1 节）验证 JCS 序列化后的 JSON 字节流（剔除 `signature` 字段）的 Ed25519 签名。
   - 若签名无效，**MUST** 拒绝。
...
4. **信任迁移引导（Migration Guidance）**：
   ...
   - 若 `new_trust_anchor` 字段存在，验证端 **SHOULD** 以醒目方式提示用户导入新的 Namespace ID 及其对应的 Trust Anchor，以便用户切换到重建后的新体系。
...
- 若所有者希望摆脱已泄露的旧 Master Secret 带来的废止风险，唯一合法路径是**迁移至全新的 Namespace ID**（见 16.2.2），而非修改协议版本号。
